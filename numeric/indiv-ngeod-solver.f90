...
contains
  subroutine ngeod_cart_rkf(r,k,om,lmax,dlmin,dlmax,tol,e,outputfile)
    ...
    real(dp), intent(in) :: lmax,om,dlmin,dlmax,tol,e
  ...
  subroutine ngeod_cart_eul(r,k,om,lmax,dl,e,outputfile)
    ! Null geodesic solver with the Euler method. ** Only for tests!! ***
    real(dp), dimension(1:3), intent(inout) :: r,k
    real(dp), intent(in) :: lmax,dl,om,e
    integer, intent(in) :: outputfile
    real(dp) :: l
